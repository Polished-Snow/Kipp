...
and placement invariance. Remaining: a **Metal 4 `mpp::tensor_ops` matmul
path** (the instruction class behind llama.cpp's remaining prefill lead on
M5-class hardware; the previously planned K-blocked matmul was measured
unnecessary — the simdgroup kernels are compute-bound, not traffic-bound),
plus a token-budget scheduler and 4-bit KV, each behind its own CPU-vs-GPU
gate. Prompt-lookup and draft-model speculative decoding plus
